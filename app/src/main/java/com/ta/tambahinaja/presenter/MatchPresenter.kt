...
class MatchPresenter (private val view: MatchView, private val database: DatabaseReference){
    ...
    fun newHighScore(auth: FirebaseAuth,total: Long, type: String, highScore: Int, oldScore: Int){
        database.child("leaderboards").child(auth.currentUser!!.uid).child("total").setValue(total+highScore-oldScore).addOnFailureListener {
